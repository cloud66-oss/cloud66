...
func (c *Client) GetConfigStoreRecord(namespace, key string) (*ConfigStoreRecord, error) {
	req, err := c.NewRequest("GET", "/configstore/namespaces/"+namespace+"/records/"+url.QueryEscape(key)+".json", nil, nil)
	if err != nil {
		return nil, err
	}
...
func (c *Client) DeleteConfigStoreRecord(namespace, key string) (*ConfigStoreRecord, error) {
	req, err := c.NewRequest("DELETE", "/configstore/namespaces/"+namespace+"/records/"+url.QueryEscape(key)+".json", nil, nil)
	if err != nil {
		return nil, err
	}
